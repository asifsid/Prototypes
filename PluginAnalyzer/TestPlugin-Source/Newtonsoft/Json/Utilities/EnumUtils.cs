	...
	internal static class EnumUtils
	{
		...
		private static EnumInfo InitializeValuesAndNames(Type enumType)
		{
			...
			for (int i = 0; i < names.Length; i++)
			{
				...
				{
					throw new InvalidOperationException("Enum name '{0}' already exists on enum '{1}'.".FormatWith(CultureInfo.InvariantCulture, text, enumType.Name));
				}
				array[i] = text;
			}
			return new EnumInfo(enumType.IsDefined(typeof(FlagsAttribute), inherit: false), array2, names, array);
		}
