	...
	internal class JsonSerializerInternalReader : JsonSerializerInternalBase
	{
		...
		private void ResolveTypeName(JsonReader reader, ref Type objectType, ref JsonContract contract, JsonProperty member, JsonContainerContract containerContract, JsonProperty containerMember, string qualifiedTypeName)
		{
			if ((member?.TypeNameHandling ?? containerContract?.ItemTypeNameHandling ?? containerMember?.ItemTypeNameHandling ?? Serializer._typeNameHandling) != 0)
			{
				...
				try
				{
					type = Serializer._serializationBinder.BindToType(typeNameKey.AssemblyName, typeNameKey.TypeName);
				}
				catch (Exception ex)
				{
		...
		private object CreateList(JsonReader reader, Type objectType, JsonContract contract, JsonProperty member, object existingValue, string id)
		{
			...
			object list2;
			if (!jsonArrayContract.ShouldCreateWrapper && existingValue is IList)
			{
				...
				try
				{
					if (contract.IsConvertable)
					{
						...
						if (contract.IsEnum)
						{
							if (value is string)
							{
								return EnumUtils.ParseEnum(contract.NonNullableUnderlyingType, value.ToString(), disallowNumber: false);
							}
