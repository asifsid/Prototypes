...
namespace Newtonsoft.Json.Serialization
{
	public class DefaultContractResolver : IContractResolver
	{
		...
		protected virtual List<MemberInfo> GetSerializableMembers(Type objectType)
		{
			...
			{
				FieldInfo fieldInfo;
				if ((object)(fieldInfo = item2 as FieldInfo) != null && !fieldInfo.IsStatic)
				{
					list.Add(item2);
				}
			}
		...
		private static void SetExtensionDataDelegates(JsonObjectContract contract, MemberInfo member)
		{
			...
			if (attribute.ReadData)
			{
				...
				if (method == null)
				{
					method = implementingType.GetMethod("Add", new Type[2] { type, type2 });
				}
		...
		protected virtual JsonContract CreateContract(Type objectType)
		{
			...
			if (!IgnoreSerializableInterface && typeof(ISerializable).IsAssignableFrom(type) && JsonTypeReflector.IsSerializable(type))
			{
